...
async function getTvDetail(idStr: string) {
  try {
    const id = parseInt(idStr, 10);
    if (isNaN(id) || id <= 0) return null;
    
    const rawDetail = await tmdbFetch<RawTmdbTvDetail>(
      `/tv/${id}?append_to_response=credits,videos,similar,content_ratings`,
      86400 // Cache for 24 hours
    );
    return normalizeTmdbTvDetail(rawDetail);
  } catch (err) {
...
export async function generateMetadata({ params }: TvPageProps): Promise<Metadata> {
  ...
  if (!tv) {
    return {
      title: 'TV Show Not Found — TheLyst',
    };
  }
  return {
    title: `${tv.title} (${tv.year || 'N/A'}) — TheLyst`,
    description: tv.overview ? tv.overview.substring(0, 160) : `Detailed tracking list metadata for ${tv.title}.`,
  };
}
        ...
        <div className="grid grid-cols-1 md:grid-cols-[300px_1fr] gap-8 md:gap-12 bg-secondary border border-border p-6 sm:p-8 rounded-xl fade-in mb-12">
          ...
          <div className="flex flex-col gap-6 w-full max-w-[300px] mx-auto md:mx-0">
            ...
            <StatusPicker
              ...
              year={tv.year}
              totalEpisodes={tv.totalEpisodes}
            />
          ...
          <div className="flex flex-col min-w-0">
            ...
            <div className="flex flex-wrap items-start justify-between gap-4 mb-2">
              ...
              {tv.aggregateScore !== null && (
                <div className="flex items-center gap-1.5 bg-rating/10 border border-rating/20 text-rating font-bold px-3 py-1 rounded text-sm sm:text-base shrink-0">
                  ...
                  {tv.aggregateScore.toFixed(1)} / 10
                </div>
              )}
            </div>
            ...
            <div className="flex flex-wrap items-center gap-2 sm:gap-3 text-xs sm:text-sm text-muted mb-6">
              <span className="font-semibold">{tv.year || '—'}</span>
              <span className="w-1.5 h-1.5 bg-border rounded-full" />
              {tv.contentRating && (
                <>
                  <span className="border border-border px-1.5 py-0.5 rounded font-mono font-semibold">{tv.contentRating}</span>
                  <span className="w-1.5 h-1.5 bg-border rounded-full" />
                </>
              )}
              {tv.runtimeMinutes && (
                <>
                  <span>{formatRuntime(tv.runtimeMinutes)} per ep</span>
                  <span className="w-1.5 h-1.5 bg-border rounded-full" />
                </>
              )}
              {tv.totalEpisodes && (
                <>
                  <span>{tv.totalEpisodes} Episodes</span>
                  <span className="w-1.5 h-1.5 bg-border rounded-full" />
                </>
              )}
              <span className="uppercase tracking-wider font-semibold">TV Show</span>
              <span className="w-1.5 h-1.5 bg-border rounded-full" />
              <a
            ...
            <p className="text-sm sm:text-base text-muted leading-relaxed mb-6">
              {tv.overview || 'No synopsis available.'}
            </p>

            {/* Watch Trailer CTA */}
            {tv.trailer?.youtubeId && (
              <div className="mb-8">
                <TrailerButton youtubeId={tv.trailer.youtubeId} />
              </div>
            )}

            {/* Crew / Creators Details */}
            {tv.creators && tv.creators.length > 0 && (
              <div className="mb-6 bg-background/30 border border-border/50 rounded-lg p-4">
                <span className="text-xs font-bold uppercase tracking-wider text-muted block mb-1">Created By</span>
                <span className="text-sm font-medium text-foreground">{tv.creators.join(', ')}</span>
              </div>
            )}
          ...
          <div className="mb-12 fade-in">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-4">
              {tv.seasons.map((season) => (
                <div key={season.seasonNumber} className="bg-secondary border border-border p-4 rounded-lg flex flex-col justify-between">
                  <div>
                    <h4 className="text-base font-semibold text-foreground">Season {season.seasonNumber}</h4>
                    <p className="text-xs text-muted mt-1">{season.episodeCount} Episodes</p>
                  </div>
                  {season.airDate && (
                    <span className="text-[10px] text-muted font-medium mt-3 block self-start">
                      Aired: {new Date(season.airDate).toLocaleDateString(undefined, { year: 'numeric', month: 'short' })}
                    </span>
                  )}
                </div>
              ))}
