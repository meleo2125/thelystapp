...
export default function HomePage() {
    ...
    const fetchTrending = async () => {
      ...
      try {
        const [moviesRes, animeRes] = await Promise.all([
          ...
          fetch('/api/media/trending?type=anime'),
        ]);
  ...
  const watchingItems = useMemo(() => {
    return list.filter((item) => item.status === 'watching' && item.type !== 'movie');
  }, [list]);
  ...
  const planToWatch = useMemo(() => {
    return list.filter((item) => item.status === 'plan_to_watch').slice(0, 6);
  }, [list]);
    ...
    try {
      const res = await fetch(`/api/list/${item.id}`, {
        ...
        body: JSON.stringify({
          ...
          status: item.cache.totalEpisodes && nextProgress === item.cache.totalEpisodes ? 'completed' : 'watching',
        }),
      });
            ...
            {listLoading ? (
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-6">
                {watchingItems.map((item) => (
                  ...
                  >
                    <Link href={`/${item.type}/${item.sourceId}`} className="relative w-14 h-20 rounded overflow-hidden border border-border flex-shrink-0 bg-background">
                      <MediaImage src={item.cache.posterPath} alt={item.cache.title} fill sizes="56px" />
                    </Link>

                    <div className="flex-1 min-w-0">
                      <Link href={`/${item.type}/${item.sourceId}`} className="font-bold text-sm text-foreground hover:text-primary transition-colors truncate block">
                        {item.cache.title}
                      </Link>
                      ...
                      <div className="flex items-center justify-between gap-3 mt-2">
                        <div className="flex-1 bg-background rounded-full h-1.5 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${item.cache.totalEpisodes ? (item.progress / item.cache.totalEpisodes) * 100 : 50}%`,
                            }}
                          />
                        </div>
            ...
            <div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-6 gap-6">
                {planToWatch.map((item) => (
                  <div key={item.id} className="group relative flex flex-col bg-secondary border border-border rounded-lg overflow-hidden media-glow w-full">
                    <Link href={`/${item.type}/${item.sourceId}`} className="relative aspect-[2/3] block bg-background">
                      <MediaImage src={item.cache.posterPath} alt={item.cache.title} fill sizes="(max-w-768px) 50vw, 200px" />
                    </Link>
                    <div className="p-3 flex-1 flex flex-col justify-between">
                      <Link href={`/${item.type}/${item.sourceId}`} className="hover:text-primary transition-colors text-xs font-bold text-foreground line-clamp-2 leading-tight">
                        {item.cache.title}
                      </Link>
                ...
                {recentlyUpdated.map((item) => (
                  <div key={item.id} className="group relative flex flex-col bg-secondary border border-border rounded-lg overflow-hidden media-glow w-full">
                    ...
                    <div className="p-3 flex-1 flex flex-col justify-between">
                      ...
                      <span className="text-[10px] text-primary-light font-bold block mt-1 uppercase">{item.status.replace(/_/g, ' ')}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          {trendingLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-6">
              {trendingMovies.slice(0, 6).map((movie) => (
                <div key={`${movie.type}-${movie.sourceId}`} className="group relative flex flex-col bg-secondary border border-border rounded-lg overflow-hidden media-glow w-full">
                  <Link href={`/movie/${movie.sourceId}`} className="relative aspect-[2/3] block bg-background">
                    <MediaImage src={movie.posterPath} alt={movie.title} fill sizes="(max-w-768px) 50vw, 200px" />
                  ...
                  <div className="p-3 flex-1 flex flex-col justify-between">
                    <Link href={`/movie/${movie.sourceId}`} className="hover:text-primary transition-colors text-xs font-bold text-foreground line-clamp-2 leading-tight">
                      {movie.title}
                    </Link>
          ...
          {trendingLoading ? (
            ...
            <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-6 gap-6">
              {trendingAnime.slice(0, 6).map((anime) => (
                <div key={`${anime.type}-${anime.sourceId}`} className="group relative flex flex-col bg-secondary border border-border rounded-lg overflow-hidden media-glow w-full">
                  <Link href={`/anime/${anime.sourceId}`} className="relative aspect-[2/3] block bg-background">
                    <MediaImage src={anime.posterPath} alt={anime.title} fill sizes="(max-w-768px) 50vw, 200px" />
                  ...
                  <div className="p-3 flex-1 flex flex-col justify-between">
                    <Link href={`/anime/${anime.sourceId}`} className="hover:text-primary transition-colors text-xs font-bold text-foreground line-clamp-2 leading-tight">
                      {anime.title}
                    </Link>
